...
- **双数据源 + 交叉验真**：天天基金（主） + 好买基金（备），自动比对两源数据：一致则直接使用，不一致则语义分析+取更准确源，且清晰标注差异原因
- **申购状态合并展示**：状态和限额合并为一个字段，如"暂停申购"、"限小额 1000元"、"开放申购"，一目了然
- **主备自动切换**：天天基金不可用自动降级到好买基金，两者都失败返回 `data_unavailable: true`，不会崩溃
- **申购状态严格校验**：开放 / 限大额 / 限小额 / 暂停，暂停时自动标记 `effectively_closed: true`
...
> **首次使用**：先选 **2) 编辑我的基金列表**，输入你持有的基金代码和名称，基金代码是 6 位数字（名称可以自定义），保存后再选 **1) 查看我的基金**。

或者选 **3) 打开可视化界面**，浏览器中会打开一个图形化页面，直接点击按钮操作，无需记忆任何命令。
...
| 申购状态 | 合并展示状态和限额，如"暂停申购"、"限小额 1000元"、"开放申购" | 暂停申购 |
| 总费率 | 管理费 + 托管费 + 销售服务费 | 0.80% |

### 申购状态说明

QDII 基金额度有限，经常会出现限购或暂停申购，这是正常现象。申购状态是**状态和限额合并展示**：

| 展示 | 含义 | 能否买入 |
|------|------|---------|
| 开放申购 | 正常买入，无限制 | 可以 |
| 限大额（xx） | 单日购买有上限（通常几万到百万） | 小额可买 |
| 限小额 xxxx元 | 单日限购金额很小（几百到几千元） | 可买但额度极少 |
| 暂停申购 | 完全不能买入 | 不可买 |

### 交叉验真

工具从天天基金（主）和好买基金（备用）两个来源获取数据，自动比对：

- **两源数据一致** → 无标记，直接展示
- **两源不一致** → 语义分析判断是否等价，以主源为准，标注差异原因
- **数值字段差异** → 标注具体差值（如"差异 3.21 百分点"），帮助判断数据可靠性
